...
### Added
- **Volume Control — a per-app volume mixer (Preview).** The Customization group's Volume Control tab is now a working mixer instead of a placeholder: it lists each application currently playing sound on your default playback device with its own volume slider, a mute button, and a live level meter, so you can turn one app down (or mute it) without touching the master volume or digging through the Windows mixer. It reads and controls audio through the documented Windows Core Audio APIs directly — no extra dependency is bundled — and does no background work while the tab isn't open. It's marked **Preview** while it settles in: this first version covers apps on the default output device; per-app output-device routing (send one app to your headset, another to speakers) and saved volume presets are planned for a later update.
...
- **Work-in-progress tabs now carry the "PREVIEW" badge like the other unfinished tabs.** Five placeholder tabs (Gaming Profile, Bandwidth Monitor, Edge/OneDrive Remover, Notification Blocker, Volume Control) showed no badge in the sidebar, so they looked indistinguishable from finished tabs even though they open an empty placeholder. They now show the same "PREVIEW" pill the other in-development tabs already carry, setting the expectation before you click.
