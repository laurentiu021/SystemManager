...
### Added
- **Gaming Profile — one-click, fully-reversible game mode.** The Gaming & Profiles group's Gaming Profile tab is now a working "game mode" instead of a placeholder: tick the optimizations you want, optionally pick the running game to target, and press Start — SysManager applies them together and restores everything automatically when the game exits (or when you press Stop). It bundles the app's already-existing, individually-audited optimizations: the Ultimate Performance power plan, reduced visual effects, the finest (~0.5 ms) timer resolution, freeing standby memory, pausing Windows Search indexing, and silencing notifications; when you select a game, its CPU priority is also raised and it's pinned to the performance cores. Everything is snapshot-based — the original power plan, visual-effects, indexing, and notification state is captured before any change and restored exactly (SysManager also tries a System Restore point first, best-effort). Because the session is recorded to its own local file, if SysManager closes mid-game the system-wide changes are offered for restore on next launch. Steps that need administrator (freeing standby memory, pausing indexing) are clearly skipped when not elevated rather than silently failing. This first version is fully reversible; closing background applications and saving named per-game profiles are planned for a later update.
...
- **Work-in-progress tabs now carry the "PREVIEW" badge like the other unfinished tabs.** The placeholder tabs (Bandwidth Monitor, Edge/OneDrive Remover, Notification Blocker) showed no badge in the sidebar, so they looked indistinguishable from finished tabs even though they open an empty placeholder. They now show the same "PREVIEW" pill the other in-development tabs already carry, setting the expectation before you click. (Volume Control and Gaming Profile also carried this placeholder badge until each was implemented — see their entries above.)
